...
def process_csv_data(folder):
    max_pollution_1 = float('-inf')
    max_pollution_2 = float('-inf')
    ...
    with open (folder, "r") as file:
        ...
        for row in data:
            ...
            list_of_polution_P1.append(current_pollution_P1)
            list_of_polution_P2.append(current_pollution_P2)
            time = row["timestamp"].split("T")
            list_of_time.append(time[1])
            list_of_dates.append(time[0])
           
            if current_pollution_P1 > max_pollution_1:
                max_pollution_1 = current_pollution_P1
            if current_pollution_P1 < min_pullution_p1:
                min_pullution_p1 = current_pollution_P1

            if current_pollution_P2 > max_pollution_2:
                max_pollution_2 = current_pollution_P2
        ...
        return {
            "max_pollution_1 ": max_pollution_1,
            "max_pollution_2 ": max_pollution_2,
            "min_pullution_p1 ": min_pullution_p1,
            "min_pullution_p2 ": min_pullution_p2,
            "average_P1": average_P1,
            "average_P2": average_P2,
            "all_pollutions_1 ": list_of_polution_P1,
            "all_pollutions_2 ": list_of_polution_P2,
        }
